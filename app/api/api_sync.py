...
def wrong_word_detect():
    ...
    print("text:",text)
    idx_errors = pycorrector.detect(text)  #返回类型是list, [error_word, begin_pos, end_pos, error_type]，pos索引位置以0开始。begin_pos=<实际位置<end_pos
    print("idx_errors:",idx_errors)
    new_detect_value=['']*len(idx_errors)
    for i in range (0,len(idx_errors)):
        new_detect_value[i]={"error_word":idx_errors[i][0],"begin_pos":idx_errors[i][1],"end_pos":idx_errors[i][2],"error_type":idx_errors[i][3]}
    data = {
        "code": 0,
...
def wrong_word_correct():
    text=request.values.get('text')
    print("text:",text)
    idx_errors = pycorrector.correct(text)  #返回类型是list, [error_word, begin_pos, end_pos, error_type]，pos索引位置以0开始。begin_pos=<实际位置<end_pos
    print("idx_errors:",idx_errors)
    ...
    for i in range (0,len(idx_errors[1])):
        new_detect_value[i]={"new_sentence":idx_errors[0],"error_word":idx_errors[1][i][0],"correct_word":idx_errors[1][i][1],"begin_pos":idx_errors[1][i][2],"end_pos":idx_errors[1][i][3]}
...
def video_cutter():
    ...
    target_video_abs_path = clip_handle(source_video_abs_path=video_path,target_video_abs_path=cutter_video_path, start_time=int(start_time),stop_time=int(stop_time))
...
def get_video_info():
    ...
    video_info=[{"video_time":str(video_length)+'s','video_width':width,'video_height':height,'file_size':file_size}]
